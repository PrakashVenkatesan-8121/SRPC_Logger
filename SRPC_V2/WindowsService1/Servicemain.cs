        ...
        private void ProcessLogQueue()
        {
            while (running)
            {
                while (logQueue.TryDequeue(out var logItem))
                {
                    try
                    {
                        File.AppendAllText(logItem.logPath, $"{DateTime.Now} [{logItem.logType}]: {logItem.message}\r\n");
                    }
                    catch (Exception ex)
                    {
                        ...
                        try
                        {
                            File.AppendAllText(DefaultLogFilePath, $"{DateTime.Now} [Critical]: Log failure for path '{logItem.logPath}'. Reason: {ex.Message}\r\n");
                        }
